...
3. Клонируйте репозиторий конфигурации:
   ```bash
   git clone <repo-url> ~/.config/nvim
   ```
4. Запустите `nvim`. Lazy.nvim автоматически установит плагины. При необходимости выполните `:Lazy sync` для синхронизации и `:Mason` для установки нужных LSP/линтеров/форматтеров.
5. Для Go убедитесь, что `golangci-lint` и `gotestsum` доступны в `$PATH`; для Python отладки требуется `debugpy`, для Markdown предпросмотра — `markdown-preview.nvim` соберётся автоматически.

## Плагины
Полный список фиксирован в `lazy-lock.json` и включает:
...
Полный перечень с ветками и коммитами смотрите в `lazy-lock.json`.
